#!/usr/bin/env python3
"""PDF Report Generator for SQL Injection Tester"""
...
class SQLiPDFGenerator:
    def __init__(self):
        ...
        self.h1_style = ParagraphStyle('H1', parent=self.styles['Heading1'], fontSize=14,
            textColor=colors.HexColor('#dc2626'), spaceAfter=8, spaceBefore=10, fontName='Helvetica-Bold')
        self.h2_style = ParagraphStyle('H2', parent=self.styles['Heading2'], fontSize=11,
            textColor=colors.HexColor('#7f1d1d'), spaceAfter=6, spaceBefore=6, fontName='Helvetica-Bold')
    ...
    def generate(self, report: Dict, output_file: str):
        ...
        elements.append(Paragraph("SQL INJECTION SECURITY REPORT", self.title_style))
        elements.append(Spacer(1, 0.3*inch))
        elements.append(Paragraph("Automated SQL Injection Assessment — SecureScope", self.styles['Normal']))
        elements.append(Spacer(1, 0.3*inch))
        for item in [
        ...
        table_data = [
            ...
            ['CRITICAL', str(sev.get('CRITICAL', 0)), 'A03:2021', 'Immediate (24h)'],
        ...
        if vulns:
            ...
            for i, v in enumerate(vulns, 1):
                ...
                elements.append(Paragraph(
                    f"<b>#{i} — {v.get('type', 'SQLi')}</b> &nbsp; <font color='{c}'>[{sev_val}]</font>",
                    self.h2_style))
