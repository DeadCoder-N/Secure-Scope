#!/usr/bin/env python3
"""
SQL Injection Tester Backend API — SecureScope Tool 2
Port: 5002
"""
...
from modules.scanner import SQLiScanner
from reports.pdf_generator import SQLiPDFGenerator
...
def build_report(findings, target, scan_mode, params_tested=None):
    ...
    for f in findings:
        sev[f.get('severity', 'HIGH')] = sev.get(f.get('severity', 'HIGH'), 0) + 1
    return {
        'target': target,
        'scan_mode': scan_mode,
...
def scan():
    try:
        ...
        json_file = temp_dir / f"sqli_{date_str}_{scan_id}.json"
        pdf_file  = temp_dir / f"sqli_{date_str}_{scan_id}.pdf"

        with open(json_file, 'w') as f:
            json.dump(report, f, indent=2)

        try:
            SQLiPDFGenerator().generate(report, str(pdf_file))
...
def health():
    return jsonify({'status': 'healthy', 'service': 'SQL Injection Tester', 'port': 5002})


if __name__ == '__main__':
    print("\n" + "="*50)
    print("💉 SQL INJECTION TESTER — SecureScope Tool 2")
    print("="*50)
