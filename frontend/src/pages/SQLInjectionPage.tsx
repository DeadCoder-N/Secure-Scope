...
export default function SQLInjectionPage() {
  ...
  const manualSteps = [
    ...
    { step: 'Testing error-based payloads...', percent: 40 },
    { step: 'Testing union-based payloads...', percent: 60 },
    { step: 'Testing boolean-blind payloads...', percent: 80 },
    { step: 'Testing time-based payloads...', percent: 95 },
  ];

  const smartSteps = [
    { step: 'Crawling target website...', percent: 15 },
    { step: 'Discovering parameters...', percent: 30 },
    { step: 'Testing error-based SQLi...', percent: 50 },
    { step: 'Testing union-based SQLi...', percent: 65 },
    { step: 'Testing blind SQLi...', percent: 80 },
    { step: 'Testing time-based SQLi...', percent: 95 },
  ];
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 sm:px-6 py-10 space-y-6">
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          <h1 className="text-2xl font-bold text-slate-900">SQL Injection Tester</h1>
          <p className="text-sm text-slate-500">Detect error-based, union-based, boolean-blind, and time-based SQLi</p>
        </div>
      </div>
      ...
      <div className="card p-1 flex gap-1 w-fit">
        <button
          ...
          className={`flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-medium transition-colors ${
            scanMode === 'manual' ? 'bg-red-600 text-white' : 'text-slate-600 hover:bg-slate-100'
          }`}
        >
        ...
        <button
          ...
          className={`flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-medium transition-colors ${
            scanMode === 'smart' ? 'bg-red-600 text-white' : 'text-slate-600 hover:bg-slate-100'
          }`}
        >
      ...
      <div className="card p-6 space-y-4">
        <div>
          ...
          <p className="text-xs text-slate-400 mt-1.5">
            {scanMode === 'manual'
              ? 'URL must contain query parameters (e.g. ?id=1&name=test)'
        ...
        <div className="space-y-5">
            ...
            <div className="flex items-center justify-between flex-wrap gap-4">
              <div className="flex items-center gap-6">
                <div className="text-center">
                  <p className={`text-3xl font-bold ${result.total_vulnerabilities > 0 ? 'text-red-600' : 'text-emerald-600'}`}>
          ...
          {result.vulnerabilities.length > 0 ? (
            ...
            <div className="card p-8 text-center">
              <p className="text-emerald-600 font-semibold text-lg">✅ No SQL injection vulnerabilities found</p>
              <p className="text-slate-500 text-sm mt-1">The target appears to be protected against SQLi.</p>
            </div>
          )}
        </div>
      )}
