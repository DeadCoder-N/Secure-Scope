...
import { Syringe, Play, AlertCircle, Globe, Link } from 'lucide-react';
...
const MANUAL_STEPS: { msg: string; type: LogLine['type']; pct: number }[] = [
  { msg: 'Parsing URL parameters...',              type: 'info',    pct: 10 },
  { msg: 'Sending baseline request...',            type: 'muted',   pct: 20 },
  { msg: 'Testing error-based payloads...',        type: 'info',    pct: 38 },
  { msg: 'Testing union-based payloads...',        type: 'info',    pct: 55 },
  { msg: 'Testing boolean-blind payloads...',      type: 'info',    pct: 70 },
  { msg: 'Testing time-based payloads...',         type: 'info',    pct: 85 },
  { msg: 'Verifying findings...',                  type: 'muted',   pct: 95 },
];
const SMART_STEPS: { msg: string; type: LogLine['type']; pct: number }[] = [
  { msg: 'Initializing crawler...',                type: 'info',    pct: 8  },
  { msg: 'Crawling target website...',             type: 'default', pct: 20 },
  { msg: 'Discovering URL parameters...',          type: 'info',    pct: 32 },
  { msg: 'Testing error-based SQLi...',            type: 'info',    pct: 48 },
  { msg: 'Testing union-based SQLi...',            type: 'info',    pct: 62 },
  { msg: 'Testing boolean-blind SQLi...',          type: 'info',    pct: 75 },
  { msg: 'Testing time-based SQLi...',             type: 'info',    pct: 88 },
  { msg: 'Verifying findings...',                  type: 'muted',   pct: 96 },
];
...
export default function SQLInjectionPage() {
  ...
  async function runScan() {
    ...
    setLogs([buildLog(`Starting SQL injection scan — mode: ${scanMode}`, 'info')]);
    setPercent(2);
    ...
    try {
      ...
      if (total > 0) {
        pushLog(`⚠ ${total} SQL injection vulnerability${total !== 1 ? 'ies' : 'y'} confirmed.`, 'warn');
        data.vulnerabilities?.forEach((v: Vuln) => {
          pushLog(`✓ ${v.type} on param '${v.parameter}' [${v.severity}]`, 'error');
        });
      } else {
        pushLog('No SQL injection vulnerabilities detected.', 'success');
      }
      setPercent(100);
      setResult(data);
    } catch (e: unknown) {
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 sm:px-6 py-10 space-y-6">
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          <h1 className="text-xl font-bold text-slate-100 tracking-tight">SQL Injection Tester</h1>
          <p className="text-xs text-slate-600">Error-based · Union-based · Boolean-blind · Time-based</p>
        </div>
      </div>
      ...
      <div className="card p-6 space-y-4">
        <div>
          ...
          <input className="input"
            placeholder={scanMode === 'manual' ? 'http://localhost:8888/user?id=1' : 'http://localhost:8888'}
            value={target} onChange={e => setTarget(e.target.value)}
            onKeyDown={e => e.key === 'Enter' && !loading && runScan()} />
          <p className="text-xs text-slate-700 mt-2">
            {scanMode === 'manual'
              ? 'URL must contain query parameters — e.g. ?id=1'
              : 'Smart Crawl discovers all pages and parameters automatically'}
          </p>
        </div>
        <button onClick={runScan} disabled={loading || !target.trim()} className="btn-danger">
          <Play size={14} />
        ...
        <div className="space-y-4 fade-in-up">
          <div className="card p-5">
            <div className="flex items-center justify-between flex-wrap gap-4">
              <div className="flex items-center gap-6">
                <div>
                  <p className={`text-3xl font-bold font-mono ${result.total_vulnerabilities > 0 ? 'text-red-400' : 'text-green-400'}`}>
          ...
          {result.vulnerabilities.length > 0 ? (
            ...
            <div className="card p-10 text-center">
              <p className="text-green-400 font-semibold">✓ No SQL injection vulnerabilities found</p>
